...
export const generateUpdateOneRequiredWithoutFieldNestedInput = (
  ...
  const filePath = path.resolve(
    dirPath,
    `${paramCase(model.name)}-update-required-one-without-${paramCase(filedIgnore.name)}-nested.input.ts`,
  );
  ...
  updateSetImports(extraModelImports, {
    moduleSpecifier: `./modules/${paramCase(model.name)}/dto/${paramCase(
      model.name,
    )}-update-required-one-without-${paramCase(filedIgnore.name)}-nested.input`,
    namedImports: new Set([`${model.name}UpdateRequiredOneWithout${pascalCase(filedIgnore.name)}NestedInput`]),
  });
